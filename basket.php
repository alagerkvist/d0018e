...
<head>
  ...
  <script language="javascript">
      ...
      if(parseInt(qty) <= maxQty && parseInt(qty) >= 0){
...
<body>
  ...
  if(isset($_SESSION['cart'])){
    foreach($_SESSION['cart'] as $id => $qty){
        ...
        id="'.$row['productID'].'" name="qty" value="'.$qty.'" /></td>

        <td><button type="button" onclick="updatecart(this, '.$row['qty'].');"
        id="'.$row['productID'].'" name="add" value="'.($qty+1).'">+</button></td>

        <td><button type="button" onclick="updatecart(this, '.$row['qty'].');"
        id="'.$row['productID'].'" name="sub" value="'.($qty-1).'">-</button></td>
